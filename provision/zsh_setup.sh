...
install_direnv() {
  ...
  mkdir ~/bin
  curl -sSL https://github.com/direnv/direnv/releases/download/v2.17.0/direnv.linux-amd64 > ~/bin/direnv
  chmod +x ~/bin/direnv
  echo -e "\n#Hook direnv environment switcher"  >> ~/.zshrc
  echo -e "eval \"\$(direnv hook zsh)\"\n" >> ~/.zshrc
}
